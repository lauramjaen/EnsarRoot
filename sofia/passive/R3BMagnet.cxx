// -------------------------------------------------------------------------
// -----                        R3BMagnet source file                  -----
// -----                  Created 26/03/09  by D.Bertini               -----
// -----	     Last modification 29/08/12 by J.L.Rodriguez       -----
// -------------------------------------------------------------------------
...
void R3BMagnet::ConstructGeometry1(){
    ...
    cout<<"-I- R3BMagnet::ConstructGeometry1()"  <<endl;
...
void R3BMagnet::ConstructGeometry2(){
   ...
   if (gGeoManager->GetMedium("Vacuum") ){
    ...
    Double_t Aladin_angle = -7.0;     // degree
    Double_t DistanceToTarget = 360.0;// cm
    Double_t Yoke_thickness = 50.;    // cm
    Double_t Correction = 0.0;      // cm
    Double_t DistanceFromtargetToAladinCenter
	     = DistanceToTarget + Correction;
  ...
  if(fMedium==2){ 
   ...
   // Volume:
   TGeoVolume* 
   magnet_log = new TGeoVolume("magnet_log",sub23s, pMedFe);
   ...
   dy = 0.00000;
   dz = 120.0;
   TGeoCombiTrans* pMatrix4w = new TGeoCombiTrans("", dx,dy,dz,0);

   // Shape: TGeoBBox
   dx = 155.1;
   dy = 52.4;
   dz = 0.0050;
   TGeoShape *windows4 = new TGeoBBox("windows4", dx/2.,dy/2.,dz/2.);

   // Volume:
   TGeoVolume*
   Windows_log4 = new TGeoVolume("Windows_log4",windows4, pMedMylar);
   Windows_log4->SetVisLeaves(kTRUE);
   Windows_log4->SetLineColor(28);

   // Position Volume
   TGeoCombiTrans* pGlobal4w = GetGlobalPosition(pMatrix4w);
   pWorld->AddNode(Windows_log4, 0, pGlobal4w);
