...
// -----   Default constructor   -------------------------------------------
R3BMWpc2::R3BMWpc2() : R3BDetector("R3BMWpc2", kTRUE, kGFI) {
  ResetParameters();
  fMWpcCollection = new TClonesArray("R3BMWpc2Point");
  fMWpcHitCollection = new TClonesArray("R3BMWpc2Hit");
  fPosIndex = 0;
  kGeoSaved = kFALSE;
  flGeoPar = new TList();
...
R3BMWpc2::R3BMWpc2(const char* name, Bool_t active) 
...
void R3BMWpc2::Initialize()
{
  FairDetector::Initialize();
   ...
   cout << endl;
   cout << "-I- R3BMWpc2: initialisation" << endl;
   cout << "-I- R3BMWpc2: Vol (McId) def." << endl;

   Char_t buffer[126];

  
	for (Int_t i=0;i<1;i++ ) {
     sprintf(buffer,"DetID_%i",i+1);
     cout << "-I- R3BMWpc2: Detector   : " << i << " connected to (McId) ---> " <<  gMC->VolId(buffer)<< endl;
...
void R3BMWpc2::SetSpecialPhysicsCuts(){

   cout << endl;

   cout << "-I- R3BMWpc2: Adding customized Physics cut ... " << endl;
   cout << "-I- R3BMWpc2: Yet not implemented !... " << endl;
...
Bool_t R3BMWpc2::ProcessHits(FairVolume* vol) {
   ...
   // Getting the Infos from mwpc2 Volumes
   ...
   Int_t mwpc2Id = 0;
	  ...
	  if(GetPlaneType(volIdPla)!=-1){
		   mwpc2Type = GetDetectorType(volId1);
		   mwpc2Copy = mwpc2Type;
		   mwpc2Id = 1+mwpc2Type;
                   //cout<< "mwpc2= "<< mwpc2Type << endl;
		   if(mwpc2Type>2) 
			   cout << "-E- R3BMWpc2: Wrong number. " << endl;
	   }
...
//TGeoNode *bla = gGeoManager->FindNextBoundary(2);
    ...
    AddHit(fTrackID, fVolumeID, mwpc2Type , mwpc2Copy , mwpc2Id,
...
void R3BMWpc2::Register() {
  FairRootManager::Instance()->Register("MWPC_2", GetName(), fMWpcCollection, kTRUE);
...
// -----   Public method Print   ----------------------------------------------
void R3BMWpc2::Print() const {
  Int_t nHits = fMWpcCollection->GetEntriesFast();
  cout << "-I- R3BMWpc2: " << nHits << " points registered in this event." 
       << endl;
  Int_t nmwpc2Hits = fMWpcHitCollection->GetEntriesFast();
  cout << "-I- R3BMWpc2: " << nmwpc2Hits << " hits registered in this event." 
	<< endl;
}
// ----------------------------------------------------------------------------
...
void R3BMWpc2::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset) {
  ...
  cout << "-I- R3BMWpc2: " << nEntries << " entries to add." << endl;
  TClonesArray& clref = *cl2;
  R3BMWpc2Point* oldpoint = NULL;
   for (Int_t i=0; i<nEntries; i++) {
   oldpoint = (R3BMWpc2Point*) cl1->At(i);
    Int_t index = oldpoint->GetTrackID() + offset;
    oldpoint->SetTrackID(index);
    new (clref[fPosIndex]) R3BMWpc2Point(*oldpoint);
    fPosIndex++;
  }
   cout << " -I- R3BMWpc2: " << cl2->GetEntriesFast() << " merged entries."
       << endl;
}

// -----   Private method AddHit   --------------------------------------------
R3BMWpc2Point* R3BMWpc2::AddHit(Int_t trackID, Int_t detID, Int_t volid , Int_t copy, Int_t ident, 
							  TVector3 posIn, TVector3 posOut, TVector3 momIn, TVector3 momOut, 
							  Double_t time, Double_t length, Double_t eLoss) {
  TClonesArray& clref = *fMWpcCollection;
  Int_t size = clref.GetEntriesFast();
  if (fVerboseLevel>1) 
    cout << "-I- R3BMWpc2: Adding Point at (" << posIn.X() << ", " << posIn.Y() 
	 << ", " << posIn.Z() << ") cm,  detector " << detID << ", track "
	 << trackID << ", energy loss " << eLoss*1e06 << " keV" << endl;
  return new(clref[size]) R3BMWpc2Point(trackID, detID, volid, copy , ident, posIn, posOut,
				      momIn, momOut, time, length, eLoss);
}

// -----   Private method Addmwpc2Hit   --------------------------------------------
R3BMWpc2Hit* R3BMWpc2::Addmwpc2Hit(Int_t type, Int_t copy, Int_t ident, Double_t energy, Double_t time) {

	TClonesArray& clref = *fMWpcHitCollection;
	Int_t size = clref.GetEntriesFast();
	if (fVerboseLevel>1) 
		cout << "-I- R3BMWpc2: Adding Hit in detector type " << type << ", and copy " << copy 
		<< " with unique identifier " << ident << ", depositing " << energy*1e06 << " keV" << endl;
	return new(clref[size]) R3BMWpc2Hit(type, copy, ident, energy, time);
}


// -----   Public method ConstructGeometry   ----------------------------------
void R3BMWpc2::ConstructGeometry() {

	

    cout << endl;
    cout << "-I- R3Bmwpc2:: ConstructGeometry() "<< endl;
    cout << endl;
   ...
   if (gGeoManager->GetMedium("vacuum") ){
	...
	TGeoVolume*
	pWorld  = new TGeoVolume("MWPC2_World",pCBWorld, pMedAr);
	
	TGeoCombiTrans *t0 = new TGeoCombiTrans(-80.7,0.0,695.57,rot_mwpc);
  ...
  // pWorld->AddNode(basesub_log7, 0, pGlobal2);
   ...
   // Volume: 
   TGeoVolume*
   Detector_log1 = new TGeoVolume("DetID_1",Detector1, pMedAr);
