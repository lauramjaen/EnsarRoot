// -------------------------------------------------------------------------
// -----                EnsarAsciiGenerator source file                 -----
// -------------------------------------------------------------------------
#include "EnsarAsciiGenerator.h"
...
// -----   Default constructor   ------------------------------------------
EnsarAsciiGenerator::EnsarAsciiGenerator()
...
EnsarAsciiGenerator::EnsarAsciiGenerator(const char* fileName)
  : fInputFile(NULL), fFileName(fileName),
    fPDG(TDatabasePDG::Instance()), fIonMap(), 
    fX(0.), fY(0.), fZ(0.), fPointVtxIsSet(kFALSE),
    fDX(0.), fDY(0.), fDZ(0.), fBoxVtxIsSet(kFALSE)
{
  cout << "-I- EnsarAsciiGenerator: Opening input file " << fileName << endl;
  // Open first the file to register all new ions.
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) 
    Fatal("EnsarAsciiGenerator","Cannot open input file.");
  cout << "-I- EnsarAsciiGenerator: Looking for ions..." << endl;

  Int_t nIons = RegisterIons();
  cout << "-I- EnsarAsciiGenerator: " << nIons << " ions registered." 
       << endl;
  CloseInput();
...
EnsarAsciiGenerator::EnsarAsciiGenerator(const EnsarAsciiGenerator& right)
...
Bool_t EnsarAsciiGenerator::ReadEvent(FairPrimaryGenerator* primGen) {

  // Check for input file
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarAsciiGenerator: Input file not open!" << endl;
    return kFALSE;
  }
  ...
  *fInputFile >> eventId >> nTracks >> pBeam >> b;
  ...
  if ( fInputFile->eof() ) {
      cout << "-I- EnsarAsciiGenerator: End of input file reached " << endl;
      CloseInput();
      return kFALSE;
  }

  cout << "-I- EnsarAsciiGenerator: Reading Event: " << eventId << ",  pBeam = "
  ...
  for (Int_t itrack=0; itrack<nTracks; itrack++) {
      ...
      *fInputFile >> iPid  >> iZ >> iA >> px >> py >> pz >> vx >> vy >> vz >> iMass;
      ...
      /*    cout << "-I- EnsarAsciiGenerator: iPid: " << iPid <<
	  ",   A = " << iA << " Z = " << iZ <<
	  " px = "  << px <<
	  " py = "  << py <<
	  " pz = "  << pz <<
      ...
      */
	  ...
	  if ( ! part ) {
	      cout << "-W- EnsarAsciiGenerator::ReadEvent: Cannot find "
...
void EnsarAsciiGenerator::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
       cout << "-I- EnsarAsciiGenerator: Closing input file " 
	    << fFileName << endl;
       fInputFile->close();
    }
    delete fInputFile;
...
Int_t EnsarAsciiGenerator::RegisterIons() {
