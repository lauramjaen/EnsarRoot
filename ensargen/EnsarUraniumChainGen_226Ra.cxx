// *********************************************************************
// *****   
// *****             EnsarUraniumChainGen_226Ra source file                                   
// *****   							
// *********************************************************************

#include "EnsarUraniumChainGen_226Ra.h"
...
// -----   Default constructor   ------------------------------------------
EnsarUraniumChainGen_226Ra::EnsarUraniumChainGen_226Ra()  :
  FairGenerator(),
...
EnsarUraniumChainGen_226Ra::EnsarUraniumChainGen_226Ra(const char* inputFile) :
  FairGenerator(),
	fPointVtxIsSet(0), fBoxVtxIsSet(0),
  fThetaRangeIsSet(0), fPhiRangeIsSet(0),
	fX(0), fY(0), fZ(0),
	...
	fPhiMin(0), fPhiMax(0)
{
  ...
  cout << "-I- EnsarUraniumChainGen_226Ra: Opening input file " << inputFile << endl;
  fFileName  = inputFile;
  
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) Fatal("EnsarUraniumChainGen_226Ra","Cannot open input file.");
...
Bool_t  EnsarUraniumChainGen_226Ra::Init()
{
		
	if (fPointVtxIsSet && fBoxVtxIsSet) {
    Fatal("Init()","EnsarUraniumChainGen_226Ra: Cannot set point and box vertices simultaneously");
  }
...
// -----   Read events   --------------------------------------------------
Bool_t EnsarUraniumChainGen_226Ra::ReadEvent(FairPrimaryGenerator* primGen)
{
  
  
  //----- Check for input file -----
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarUraniumChainGen_226Ra: Input file not open!" << endl;
    return kFALSE;
  }
	...
	Probability_emmittedParticles [0]=0.9384;
	Probability_emmittedParticles [1]=0.0616;
	...
	while(finalstate>0){
		...
		if (Energy_gammas_array->GetAt(0) > 0.000186211){
				cout<<"		Ading gamma of energy="<<Energy_gammas_array->GetAt(0)<<" p="<<px<<","<<py<<","<<pz<<endl;
		}
...
void EnsarUraniumChainGen_226Ra::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
      cout << "-I- EnsarUraniumChainGen_226Ra: Closing input file " 
	   << fFileName << endl;
      fInputFile->close();
    }
    delete fInputFile;
...
// ---Read Parameters File --------------------------------------------
void EnsarUraniumChainGen_226Ra::ReadParameters() {

}

// --- De-excitation Process --------------------------------------------
TArrayF* EnsarUraniumChainGen_226Ra::Deexcitation(Int_t InicialState, Int_t & FinalState) {
...
void EnsarUraniumChainGen_226Ra::TrackMomentum( Double_t Energy, Double_t & Px, Double_t & Py, Double_t & Pz ) {
...
ClassImp(EnsarUraniumChainGen_226Ra)
