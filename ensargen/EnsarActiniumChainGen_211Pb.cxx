// *********************************************************************
// *****   
// *****             EnsarActiniumChainGen_211Pb source file                                   
// *****   							
// *********************************************************************

#include "EnsarActiniumChainGen_211Pb.h"
...
// -----   Default constructor   ------------------------------------------
EnsarActiniumChainGen_211Pb::EnsarActiniumChainGen_211Pb()  :
  FairGenerator(),
...
EnsarActiniumChainGen_211Pb::EnsarActiniumChainGen_211Pb(const char* inputFile) :
  FairGenerator(),
	fPointVtxIsSet(0), fBoxVtxIsSet(0),
  fThetaRangeIsSet(0), fPhiRangeIsSet(0),
	fX(0), fY(0), fZ(0),
	...
	fPhiMin(0), fPhiMax(0)
{
  ...
  cout << "-I- EnsarActiniumChainGen_211Pb: Opening input file " << inputFile << endl;
  fFileName  = inputFile;
  
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) Fatal("EnsarActiniumChainGen_211Pb","Cannot open input file.");
...
Bool_t  EnsarActiniumChainGen_211Pb::Init()
{
		
	if (fPointVtxIsSet && fBoxVtxIsSet) {
    Fatal("Init()","EnsarActiniumChainGen_211Pb: Cannot set point and box vertices simultaneously");
  }
...
// -----   Read events   --------------------------------------------------
Bool_t EnsarActiniumChainGen_211Pb::ReadEvent(FairPrimaryGenerator* primGen)
{
  
  
  //----- Check for input file -----
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarActiniumChainGen_211Pb: Input file not open!" << endl;
    return kFALSE;
  }
	...
	Probability_emmittedParticles [0]=0.9132;
	Probability_emmittedParticles [1]=0.0163;
	Probability_emmittedParticles [2]=0.0628;
...
void EnsarActiniumChainGen_211Pb::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
      cout << "-I- EnsarActiniumChainGen_211Pb: Closing input file " 
	   << fFileName << endl;
      fInputFile->close();
    }
    delete fInputFile;
...
// ---Read Parameters File --------------------------------------------
void EnsarActiniumChainGen_211Pb::ReadParameters() {

}

// --- De-excitation Process --------------------------------------------
TArrayF* EnsarActiniumChainGen_211Pb::Deexcitation(Int_t InicialState, Int_t & FinalState) {
	...
	//Excited states
	Energygammas				[1][1]=0.000404853;
	Probability					[1][1]=1.;
	NucleusFinalState		[1][1]=0;



	Energygammas				[2][1]=0.000362072;
	Probability					[2][1]=0.0645;
	NucleusFinalState		[2][1]=1;

	Energygammas				[2][2]=0.00076651;
	Probability					[2][2]=0.9355;
	NucleusFinalState		[2][2]=0;



	Energygammas				[3][1]=0.00006542;
	Probability					[3][1]=0.0145;
	NucleusFinalState		[3][1]=2;

	Energygammas				[3][2]=0.000427088;
	Probability					[3][2]=0.3276;
	NucleusFinalState		[3][2]=1;

	Energygammas				[3][3]=0.00083201;
	Probability					[3][3]=0.6579;
	...
	if (InicialState==1){numGammas=1;}
		...
		cout << "-E- EnsarActiniumChainGen_211Pb: Incorrect final state!" << endl;
	}
...
void EnsarActiniumChainGen_211Pb::TrackMomentum( Double_t Energy, Double_t & Px, Double_t & Py, Double_t & Pz ) {
...
ClassImp(EnsarActiniumChainGen_211Pb)
