// *********************************************************************
// *****   
// *****             EnsarThoriumChainGen_208Tl source file                                   
// *****   							
// *********************************************************************

#include "EnsarThoriumChainGen_208Tl.h"
...
// -----   Default constructor   ------------------------------------------
EnsarThoriumChainGen_208Tl::EnsarThoriumChainGen_208Tl()  :
  FairGenerator(),
...
EnsarThoriumChainGen_208Tl::EnsarThoriumChainGen_208Tl(const char* inputFile) :
  FairGenerator(),
	fPointVtxIsSet(0), fBoxVtxIsSet(0),
  fThetaRangeIsSet(0), fPhiRangeIsSet(0),
	fX(0), fY(0), fZ(0),
	...
	fPhiMin(0), fPhiMax(0)
{
  ...
  cout << "-I- EnsarThoriumChainGen_208Tl: Opening input file " << inputFile << endl;
  fFileName  = inputFile;
  
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) Fatal("EnsarThoriumChainGen_208Tl","Cannot open input file.");
...
Bool_t  EnsarThoriumChainGen_208Tl::Init()
{
		
	if (fPointVtxIsSet && fBoxVtxIsSet) {
    Fatal("Init()","EnsarThoriumChainGen_208Tl: Cannot set point and box vertices simultaneously");
  }
...
// -----   Read events   --------------------------------------------------
Bool_t EnsarThoriumChainGen_208Tl::ReadEvent(FairPrimaryGenerator* primGen)
{
  
  
  //----- Check for input file -----
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarThoriumChainGen_208Tl: Input file not open!" << endl;
    return kFALSE;
  }
	...
	Probability_emmittedParticles [0]=0.0318;
	Probability_emmittedParticles [1]=0.242;
	Probability_emmittedParticles [2]=0.222;
	Probability_emmittedParticles [3]=0.491;


	Double_t FinalState_daughter [numInicialCases];
	FinalState_daughter [0]=5;
	FinalState_daughter [1]=4;
	FinalState_daughter [2]=3;
	FinalState_daughter [3]=2;
...
void EnsarThoriumChainGen_208Tl::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
      cout << "-I- EnsarThoriumChainGen_208Tl: Closing input file " 
	   << fFileName << endl;
      fInputFile->close();
    }
    delete fInputFile;
...
// ---Read Parameters File --------------------------------------------
void EnsarThoriumChainGen_208Tl::ReadParameters() {

}

// --- De-excitation Process --------------------------------------------
TArrayF* EnsarThoriumChainGen_208Tl::Deexcitation(Int_t InicialState, Int_t & FinalState) {
	...
	Energygammas				[1][1]=0.002614511;
	Probability					[1][1]=1.;
	NucleusFinalState		[1][1]=0;

	//2nd excited state with 1 gamma
	Energygammas				[2][1]=0.000583187;
	Probability					[2][1]=1.;
	NucleusFinalState		[2][1]=1;

	//3rd excited state with 2 gammas
	Energygammas				[3][1]=0.000277371;
	Probability					[3][1]=0.33687;
	NucleusFinalState		[3][1]=2;

	Energygammas				[3][2]=0.000860557;
	Probability					[3][2]=0.66313;
	NucleusFinalState		[3][2]=1;

	//4th excited state with 3 gammas
	Energygammas				[4][1]=0.00023333;
	Probability					[4][1]=0.01319;
	NucleusFinalState		[4][1]=3;

	Energygammas				[4][2]=0.00051074;
	Probability					[4][2]=0.96984;
	NucleusFinalState		[4][2]=2;

	Energygammas				[4][3]=0.00109395;
	Probability					[4][3]=0.01697;
	NucleusFinalState		[4][3]=1;

	//5th excited state with 3 gammas
	Energygammas				[5][1]=0.000252755;
	Probability					[5][1]=0.27202;
	NucleusFinalState		[5][1]=4;

	Energygammas				[5][2]=0.00048595;
	Probability					[5][2]=0.01776;
	...
	else if (InicialState==3){numGammas=2;}
	else if (InicialState==4 || InicialState==5){numGammas=3;}
	else{
		cout << "-E- EnsarThoriumChainGen_208Tl: Incorrect final state!" << endl;
	}
...
void EnsarThoriumChainGen_208Tl::TrackMomentum( Double_t Energy, Double_t & Px, Double_t & Py, Double_t & Pz ) {
...
ClassImp(EnsarThoriumChainGen_208Tl)
