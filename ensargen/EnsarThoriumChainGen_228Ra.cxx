// *********************************************************************
// *****   
// *****             EnsarThoriumChainGen_228Ra source file                                   
// *****   							
// *********************************************************************

#include "EnsarThoriumChainGen_228Ra.h"
...
// -----   Default constructor   ------------------------------------------
EnsarThoriumChainGen_228Ra::EnsarThoriumChainGen_228Ra()  :
  FairGenerator(),
...
EnsarThoriumChainGen_228Ra::EnsarThoriumChainGen_228Ra(const char* inputFile) :
  FairGenerator(),
	fPointVtxIsSet(0), fBoxVtxIsSet(0),
  fThetaRangeIsSet(0), fPhiRangeIsSet(0),
	fX(0), fY(0), fZ(0),
	...
	fPhiMin(0), fPhiMax(0)
{
  ...
  cout << "-I- EnsarThoriumChainGen_228Ra: Opening input file " << inputFile << endl;
  fFileName  = inputFile;
  
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) Fatal("EnsarThoriumChainGen_228Ra","Cannot open input file.");
...
Bool_t  EnsarThoriumChainGen_228Ra::Init()
{
		
	if (fPointVtxIsSet && fBoxVtxIsSet) {
    Fatal("Init()","EnsarThoriumChainGen_228Ra: Cannot set point and box vertices simultaneously");
  }
...
// -----   Read events   --------------------------------------------------
Bool_t EnsarThoriumChainGen_228Ra::ReadEvent(FairPrimaryGenerator* primGen)
{
  
  
  //----- Check for input file -----
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarThoriumChainGen_228Ra: Input file not open!" << endl;
    return kFALSE;
  }
	...
	Probability_emmittedParticles [0]=0.3;
	Probability_emmittedParticles [1]=0.2;
	Probability_emmittedParticles [2]=0.4;
	Probability_emmittedParticles [3]=0.1;

	Double_t FinalState_daughter [numInicialCases];
	FinalState_daughter [0]=4;
	FinalState_daughter [1]=3;
	FinalState_daughter [2]=2;
	FinalState_daughter [3]=1;
...
void EnsarThoriumChainGen_228Ra::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
      cout << "-I- EnsarThoriumChainGen_228Ra: Closing input file " 
	   << fFileName << endl;
      fInputFile->close();
    }
    delete fInputFile;
...
// ---Read Parameters File --------------------------------------------
void EnsarThoriumChainGen_228Ra::ReadParameters() {

}

// --- De-excitation Process --------------------------------------------
TArrayF* EnsarThoriumChainGen_228Ra::Deexcitation(Int_t InicialState, Int_t & FinalState) {
	...
	Energygammas				[1][1]=0.00000628;
	Probability					[1][1]=1.;
	NucleusFinalState		[1][1]=0;

	//2nd excited state with 1 gamma
	Energygammas				[2][1]=0.00000667;
	Probability					[2][1]=1.;
	NucleusFinalState		[2][1]=0;

	//3rd excited state with 1 gamma
	Energygammas				[3][1]=0.00001352;
	Probability					[3][1]=1.;
	NucleusFinalState		[3][1]=2;

	//4rd excited state with 2 gammas
	Energygammas				[4][1]=0.00001275;
	Probability					[4][1]=0.5;
	NucleusFinalState		[4][1]=3;

	Energygammas				[4][2]=0.0000264;
	Probability					[4][2]=0.5;
	...
	if (InicialState==1 || InicialState==2 || InicialState==3){numGammas=1;}
	else if (InicialState==4){numGammas=2;}
	else{
		cout << "-E- EnsarThoriumChainGen_228Ra: Incorrect final state!" << endl;
	}
...
void EnsarThoriumChainGen_228Ra::TrackMomentum( Double_t Energy, Double_t & Px, Double_t & Py, Double_t & Pz ) {
...
ClassImp(EnsarThoriumChainGen_228Ra)
