// *********************************************************************
// *****   
// *****             EnsarUraniumChainGen_210Pb source file                                   
// *****   							
// *********************************************************************

#include "EnsarUraniumChainGen_210Pb.h"
...
// -----   Default constructor   ------------------------------------------
EnsarUraniumChainGen_210Pb::EnsarUraniumChainGen_210Pb()  :
  FairGenerator(),
...
EnsarUraniumChainGen_210Pb::EnsarUraniumChainGen_210Pb(const char* inputFile) :
  FairGenerator(),
	fPointVtxIsSet(0), fBoxVtxIsSet(0),
  fThetaRangeIsSet(0), fPhiRangeIsSet(0),
	fX(0), fY(0), fZ(0),
	...
	fPhiMin(0), fPhiMax(0)
{
  ...
  cout << "-I- EnsarUraniumChainGen_210Pb: Opening input file " << inputFile << endl;
  fFileName  = inputFile;
  
  fInputFile = new ifstream(fFileName);
  if ( ! fInputFile->is_open() ) Fatal("EnsarUraniumChainGen_210Pb","Cannot open input file.");
...
Bool_t  EnsarUraniumChainGen_210Pb::Init()
{
		
	if (fPointVtxIsSet && fBoxVtxIsSet) {
    Fatal("Init()","EnsarUraniumChainGen_210Pb: Cannot set point and box vertices simultaneously");
  }
...
// -----   Read events   --------------------------------------------------
Bool_t EnsarUraniumChainGen_210Pb::ReadEvent(FairPrimaryGenerator* primGen)
{
  
  
  //----- Check for input file -----
  if ( ! fInputFile->is_open() ) {
    cout << "-E- EnsarUraniumChainGen_210Pb: Input file not open!" << endl;
    return kFALSE;
  }
	...
	Probability_emmittedParticles [0]=0.16;
	Probability_emmittedParticles [1]=0.84;
...
void EnsarUraniumChainGen_210Pb::CloseInput() {
  if ( fInputFile ) {
    if ( fInputFile->is_open() ) {
      cout << "-I- EnsarUraniumChainGen_210Pb: Closing input file " 
	   << fFileName << endl;
      fInputFile->close();
    }
    delete fInputFile;
...
// ---Read Parameters File --------------------------------------------
void EnsarUraniumChainGen_210Pb::ReadParameters() {

}

// --- De-excitation Process --------------------------------------------
TArrayF* EnsarUraniumChainGen_210Pb::Deexcitation(Int_t InicialState, Int_t & FinalState) {
	...
	Probability					[1][1]=1.;
	Energygammas				[1][1]=0.000046539;//MeV 
	...
	if (InicialState==1){numGammas=1;}
		...
		cout << "-E- EnsarUraniumChainGen_210Pb: Incorrect final state!" << endl;
	}
...
void EnsarUraniumChainGen_210Pb::TrackMomentum( Double_t Energy, Double_t & Px, Double_t & Py, Double_t & Pz ) {
...
ClassImp(EnsarUraniumChainGen_210Pb)
