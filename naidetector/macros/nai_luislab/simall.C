            ...
            TString InFile = "evt_gen.dat")
{
  ...
  if (fGenerator.CompareTo("box") == 0  ) {
    ...
    Double32_t momentum = 0.001;
    FairBoxGenerator* boxGen = new FairBoxGenerator(pdgId, 1);
    boxGen->SetPRange(momentum, momentum);
    boxGen->SetThetaRange(0.0, 0.0);// polar angle distribution
    boxGen->SetPhiRange(0., 360.);		// azimuthal angle
  ...
  if (fGenerator.CompareTo("ascii") == 0  ) {
	...
	if (fGenerator.CompareTo("Potassium") == 0  ) {
	  EnsarPotassiumGen* Potassium = new EnsarPotassiumGen("dummy.dat");
		...
		Potassium->SetBoxXYZ(4,-10,4,-4,-10,-4);
