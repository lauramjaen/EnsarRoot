...
void tragaldabas(const char* geoTag = "")
{
  ...
  FairGeoMedium* mMethacrylate      = geoMedia->getMedium("methacrylate");
  if ( ! mMethacrylate ) Fatal("Main", "FairMedium Methacrylate not found");
  geoBuild->createMedium(mMethacrylate);
  TGeoMedium* pMethacrylateMedium = gGeoMan->GetMedium("methacrylate");
  if ( ! pMethacrylateMedium ) Fatal("Main", "Medium Methacrylate not found");
 ...
 //PCB (box) between copper and the metacrilate
  ...
  TGeoVolume *RPC_MetaBox=gGeoManager->MakeBox("RPC_MetaBox", pMethacrylateMedium, 	
