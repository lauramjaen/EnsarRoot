...
void laminaU(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/tragaldabas/geometry/laminaUalfa_";
  ...
  FairGeoMedium* mUranium      = geoMedia->getMedium("uranium");
  if ( ! mUranium ) Fatal("Main", "FairMedium uranium not found");
  geoBuild->createMedium(mUranium);
  TGeoMedium* pUraniumMedium = gGeoMan->GetMedium("uranium");
  if ( ! pUraniumMedium ) Fatal("Main", "Medium uranium not found");
...
TGeoVolume *RPC_Uranium=gGeoManager->MakeBox("RPC_Uranium", pUraniumMedium,
  		10.0/2,10.0/2,20.0/2); 
...
//  pTragaWorld->AddNode(RPC_KeepInVolume,1,new TGeoCombiTrans(0.,0.,-100.,rotUni));
  pTragaWorld->AddNode(RPC_Uranium,0,new TGeoCombiTrans(0.,0.,0.,rotUni));
  pTragaWorld->AddNode(RPC_KeepInVolume,0,new TGeoCombiTrans(0.,0.,30.,rotUni)); 
