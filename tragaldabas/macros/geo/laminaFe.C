...
void laminaFe(const char* geoTag)
{
  ...
  TString geoFileName = geoPath + "/tragaldabas/geometry/laminaFealfa_";
  ...
  TGeoMedium* pIronMedium = gGeoMan->GetMedium("iron");
  if ( ! pIronMedium ) Fatal("Main", "Medium iron not found");
...
TGeoVolume *RPC_Iron=gGeoManager->MakeBox("RPC_Iron", pIronMedium,
...
//  pTragaWorld->AddNode(RPC_KeepInVolume,1,new TGeoCombiTrans(0.,0.,-100.,rotUni));
  pTragaWorld->AddNode(RPC_Iron,0,new TGeoCombiTrans(0.,0.,0.,rotUni));
