	   ...
	   )
{
  ...
  TString simdir = dir + "/template_silicon/macros";
...
// Output files
  ...
  if (fDetList.FindObject("EnsarSiliconDet") )   {
      //My Detector definition
      EnsarDetector* mydet = new EnsarSilicon("EnsarSilicon", kTRUE);
      ...
      run->AddModule(mydet);
      EnsarSiliconDigitizer *mydigi = new EnsarSiliconDigitizer();
      run->AddTask(mydigi);
  }
