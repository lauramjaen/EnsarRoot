// -------------------------------------------------------------------------
// -----                EnsarMCTracks source file                      -----
// -----            Created 21/09/2012  by P.Cabanelas                 -----
// -------------------------------------------------------------------------

#include "EnsarMCTracks.h"
...
// -----   Default constructor   -------------------------------------------
EnsarMCTracks::EnsarMCTracks() 
{
}

EnsarMCTracks::EnsarMCTracks(const char* name, Int_t iVerbose) 
  : FairMCTracks(name, iVerbose)
{
}

InitStatus EnsarMCTracks::Init(){
  if(fVerbose>1) cout<<  "EnsarMCTracks::Init()" << endl;
   ...
   if(fVerbose>2) cout<<  "FairMCTracks::Init()  create propagator" << endl;
    ...
    fEventManager =EnsarEventManager::Instance();
...
void EnsarMCTracks::Exec(Option_t* option)
{
  if (IsActive()){
    ...
    for (Int_t i=0; i<fTrackList->GetEntriesFast(); i++)	{
	 ...
	 fEventManager->GetCurrentPDG()!= tr->GetPDG())continue;
      ...
      ((EnsarEventManager*) fEventManager)->AddParticlesToPdgDataBase(tr->GetPDG());	
      ...
      if (((EnsarEventManager*) fEventManager)->IsScaleByEnergy())
        {
